// лабораторная1_вопрос2.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
{
    int d[10000];//ЗДЕСЬ НУЖНО МЕНЯТЬ РАЗМЕР МАССИВА ПРИ ИЗМЕНЕНИИ X, Y
    ...
    for (int p = 0; p < x*y; p++)
    {
        ...
        {
            int v = rand() % 4;
            if (v == 0 && p / y + 1 >= 2 && d[p - y] == 0) { d[p - y] = 1; d[p] = 0; }
            if (v == 1 && p % y + 1 >= 2 && d[p - 1] == 0) { d[p - 1] = 1; d[p] = 0; }
            if (v == 2 && p / y + 1 <= x-1 && d[p + y] == 0) { d[p + y] = 1; d[p] = 0; }
            if (v == 3 && p % y + 1 <= y-1 && d[p + 1] == 0) { d[p + 1] = 1; d[p] = 0; }
        }
    }
...
double work(int x, int y, int a)
{
    ...
    {
        int j[10000];//ЗДЕСЬ НУЖНО МЕНЯТЬ РАЗМЕР МАССИВА ПРИ ИЗМЕНЕНИИ X, Y
